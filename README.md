...
2. A scaled frame structure used in a physical experiment ([Example 2](./Example2.md))
3. The Venus de Milo statue ([Example 3](./Example3.md))

The first example demonstrates how to use MATLAB and Simulink to conduct reduced-order analysis for rocking problems. The second example provides a reference model for building a structural system with multiple interconnected components, such as a frame, in Simulink. The last example shows the framework is able to capture the impact-induced stress wave and indicate related stress failure.
...
2. **Allows reconstruction of full-order deformation, strain, stress, and global displacements from Simulink simulation results**, whereas MATLAB’s implementation only returns global displacements of the interface nodes in Simulink.
...
- Example 2: [Rocking Frame Experiment](./Example2.md)
